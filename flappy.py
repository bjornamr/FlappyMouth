...
def main():
    ...
    pygame.display.set_caption('Flappy Mouth')
...
def showGameOverScreen(crashInfo):
    ...
    while True:
        ...
        elif (image_counter < face_count):
            if (time.time() - start > 0.4 + 0.1*image_counter):
                SOUNDS['hit'].play()
                ...
                start = time.time()
                image_counter += 1
        elif mouth_opened_times > 2:
            return
        else:
            mouth_opened = MOUTH.detect_timed()

            if mouth_opened == True:
                    mouth_opened_times += 1
                    SOUNDS['wing'].play()
